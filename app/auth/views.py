...
def change_email():
    ...
    if form.validate_on_submit():
        ...
        if user:
            if form.old_email.data == form.email.data:
                flash("the old email can't be equal to new email!")
            elif form.email.data == current_app.config["FLASK_ADMIN"]:
                flash("you can't change the old email to administrator's email")
            else:
                token = user.generate_change_email(form.email.data)
                send_email(user.email,"Change Your Email",
